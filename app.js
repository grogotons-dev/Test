...
const AdsGramManager = {
    ...
    dailyLimit: 100, // Увеличиваем лимит для AdsGram
    ...
    updateAdButtonState(isAvailable = false) {
        ...
        // Проверяем лимит - для AdsGram используем увеличенный лимит
        const hasLimit = state.adsToday >= this.dailyLimit;
        
        if (hasLimit) {
            adButton.disabled = true;
            adButton.textContent = `🎬 Лимит (${state.adsToday}/${this.dailyLimit})`;
            adButton.style.opacity = '0.6';
            adButton.style.background = 'linear-gradient(135deg, #666, #999)';
    ...
    async showRewardedAd() {
        ...
        // Проверяем лимит (увеличенный для AdsGram)
        if (state.adsToday >= this.dailyLimit) {
            this.showAdError(`Достигнут дневной лимит: ${this.dailyLimit} просмотров`);
            return false;
        }
    ...
    async sendAdWatchedEvent() {
        ...
        try {
            ...
            // Можно отправить на ваш сервер
            // fetch('ваш-сервер.com/track-ad', { method: 'POST', body: JSON.stringify(adData) })
...
const GRK_PER_AD = 15;
const ADS_LIMIT = 20; // Лимит для обычной рекламы
...
let state = JSON.parse(localStorage.getItem("grok_final")) || {
    ...
    videoAdsToday: 0, // Отдельный счетчик для видео рекламы
    lastVideoAdDay: null
};
...
function renderTasks() {
    checkAdDay();
    checkVideoAdDay(); // Проверяем отдельно видео рекламу
    
    const adText = state.adsToday >= ADS_LIMIT 
        ? t('limitReached')
        : `${t('watchAdBtn')} (+${GRK_PER_AD} ${t('currency')})`;
    
    // Для видео рекламы используем увеличенный лимит
    const videoAdsLeft = AdsGramManager.dailyLimit - state.videoAdsToday;
        ...
        <div style="text-align: center;">
            ...
            <div class="shop-card" style="max-width: 280px; margin: 16px auto; background: rgba(59, 130, 246, 0.1); border-color: rgba(59, 130, 246, 0.3);">
                ...
                <p style="font-size: 12px; margin: 8px 0; color: #3b82f6; font-weight: 600;">
                    Осталось сегодня: ${videoAdsLeft}/${AdsGramManager.dailyLimit}
                </p>
    ...
    setTimeout(() => {
        if (AdsGramManager.isInitialized) {
            ...
            const adButton = document.getElementById('watchVideoAdBtn');
            if (adButton && state.videoAdsToday < AdsGramManager.dailyLimit) {
                adButton.disabled = false;
...
async function watchVideoAd() {
    ...
    // Проверяем лимит для видео рекламы
    if (state.videoAdsToday >= AdsGramManager.dailyLimit) {
        BackgroundMusic.playCoinSound();
        showParticleEffect(`🎬 Достигнут лимит: ${AdsGramManager.dailyLimit} видео в день`);
        return;
    }
    ...
    if (success) {
        // Увеличиваем счетчик видео рекламы
        state.videoAdsToday++;
        save();
        console.log(`🎬 Видео реклама #${state.videoAdsToday} просмотрена`);
    } else {
        // Если AdsGram не сработал, показываем обычную рекламу как fallback
...
function checkVideoAdDay() {
    const today = new Date().toDateString();
    if (state.lastVideoAdDay !== today) {
        state.lastVideoAdDay = today;
        state.videoAdsToday = 0;
        save();
    }
}
