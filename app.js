...
/* ========= АУДИО-МЕНЕДЖЕР С ФОНОВОЙ МУЗЫКОЙ ========= */
const AudioManager = {
  ctx: null,
  musicEnabled: localStorage.getItem('music_enabled') !== 'false', // по умолчанию включено
  sfxEnabled: localStorage.getItem('sfx_enabled') !== 'false', // по умолчанию включено
  ...
  // Мелодия для фоновой музыки (гармоничные аккорды)
  melody: [
    { freq: 523.25, duration: 2, type: 'sine' }, // C5
    { freq: 659.25, duration: 2, type: 'sine' }, // E5
    { freq: 783.99, duration: 2, type: 'sine' }, // G5
    { freq: 880.00, duration: 2, type: 'sine' }, // A5
    { freq: 783.99, duration: 2, type: 'sine' }, // G5
    { freq: 659.25, duration: 2, type: 'sine' }, // E5
    { freq: 587.33, duration: 2, type: 'sine' }, // D5
    { freq: 493.88, duration: 2, type: 'sine' }, // B4
  ],
  ...
  playBuy() {
    this.playTone(500, 0.15, 'sawtooth', 0.3);
  },
  ...
  playNotification() {
    this.playTone(1000, 0.15, 'square', 0.25);
  },
  ...
  playMusicNote() {
    ...
    try {
      ...
      oscillator.frequency.setValueAtTime(note.freq, this.ctx.currentTime);
  ...
  setMusicVolume(value) {
    ...
    if (this.musicVolume > 0 && this.musicEnabled) {
      this.startBackgroundMusic();
...
function showParticleEffect(text) {
  ...
  let color = '#22c55e';
  if (text.includes('❌') || text.includes('Недостаточно') || text.includes('Insufficient')) color = '#ef4444';
