...
window.addEventListener('DOMContentLoaded', function() {
  // Ждем 2 секунды и скрываем splash
  setTimeout(function() {
    ...
    if (app) {
      ...
      setTimeout(() => {
        ...
        // Автоматически кликаем по кнопке Главная для активации
        const homeButton = document.querySelector('.bottom-nav button.active');
        if (homeButton) {
          homeButton.click();
        }
      }, 100);
    ...
    autoPlay() {
        ...
        if (playPromise !== undefined) {
            playPromise.then(() => {
                ...
                // Пытаемся запустить после клика пользователя
                const startOnClick = () => {
                    this.play();
...
/* ========= ЗАПУСК ПРИЛОЖЕНИЯ ========= */
document.addEventListener('DOMContentLoaded', () => {
  // После загрузки DOM запускаем основную инициализацию
  setTimeout(() => {
      updateLanguage();
      renderHeader();
